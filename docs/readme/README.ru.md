...
Список городов IANA используется для выбора вводимых вручную часовых поясов. Отображаемый город не обязан быть в этом списке; если для него нет отдельной записи IANA, выберите город IANA в том же часовом поясе. В проекте используется версия IANA 2026b.

Список городов IANA: [список городов IANA](https://data.iana.org/time-zones/tzdb-2026b/zone.tab)

### Настройка своего города

У своего города есть три необязательных значения; все их можно оставить пустыми:

| Параметр | Как заполнить | Если оставить пустым |
| --- | --- | --- |
| Мой часовой пояс | Часовой пояс IANA, например `Asia/Shanghai` | Автоматически определяется по системному часовому поясу |
| Мой город | Название города для отображения | Автоматически выводится из часового пояса |
| Мои координаты | `широта,долгота`; для южной широты и западной долготы используйте отрицательные значения | Автоматически выводятся из часового пояса |

В IANA нет отдельной записи для Пекина. Пекин использует часовой пояс `Asia/Shanghai`, поэтому для отображения Пекина укажите:
